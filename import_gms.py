...
def create_armature(model, context, coordinate_system='BLENDER', scale=0.01):
    ...
    for bone_idx, bone_data in enumerate(model.bones):
        ...
        # Build local transform ENTIRELY in PSP space
        if bone_data.rotation_quaternion:
            q = bone_data.rotation_quaternion
            quat_psp = mathutils.Quaternion((q.w, q.x, q.y, q.z))
            rot_mat_psp = quat_psp.to_matrix().to_4x4()
        else:
            rot_psp = bone_data.rotation
            rot_mat_psp = mathutils.Euler((rot_psp.x, rot_psp.y, rot_psp.z), 'XYZ').to_matrix().to_4x4()
        
        # Scale translation in PSP space
        ...
        # Build local matrix in PSP space
        local_trans_matrix = mathutils.Matrix.Translation(scaled_trans_psp)
        local_matrix_psp = local_trans_matrix @ rot_mat_psp
        
        # Compose into world (PSP space)
        if bone_data.parent_name:
        ...
        # Extract world position (PSP)
        world_pos_psp = world_matrix_psp.translation
        
        # Convert position to Blender using SAME simple conversion as mesh
        # (x, y, z) -> (x, z, -y)
        ...
        # Extract rotation from world_matrix_psp
        world_rot_mat_psp = world_matrix_psp.to_3x3()
        quat_psp = world_rot_mat_psp.to_quaternion()
        
        # Convert quaternion components same as position: (x,y,z) -> (x,z,-y)
        # Keep w unchanged
        if coordinate_system == 'BLENDER':
            quat_blender = mathutils.Quaternion((quat_psp.w, quat_psp.x, quat_psp.z, -quat_psp.y))
        else:
            quat_blender = quat_psp
        
        # Build transform
        tfm = mathutils.Matrix.Translation(world_pos_blender)
        tfm = tfm @ quat_blender.to_matrix().to_4x4()
        
        # Compute head/tail
        head = world_pos_blender
        x_axis = tfm.to_3x3() @ mathutils.Vector((1, 0, 0))
        tail = head + x_axis.normalized() * max(0.01, scale * 5.0)
        
        edit_bone.head = head
        edit_bone.tail = tail
        
        bone_map[bone_name] = (edit_bone, world_matrix_psp)
        utils.log_debug(f"  Created with head={head}, tail={tail}")
    
    # Adjust tails to point at children
    for bone_idx, bone_data in enumerate(model.bones):
